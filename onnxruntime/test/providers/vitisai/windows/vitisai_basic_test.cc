...
struct uploadReadback {
  float upload, readback, memcpyElapsedTime;
};

template <typename TElement>
uploadReadback VAIExecutionProviderTest(TElement* inputs, int x, int y, int z) {
  ...
  bufferGPU = vaip::tensor_proto_new_d3d12_cpu_to_gpu(d3d12_device.Get(), bufferUploader, cmdList.Get(),
                                                      static_cast<void*>(inputs), byteSize);
  int uploadElapsed = vaip::getElapsedTime(start);

  // copy GPU buffer to CPU (readback)
  void* output = nullptr;
  int memcpyElapsedTime;

  // calling GPU->CPU readback function and measuring time
  start = vaip::getStartingTime();
  // copy GPU buffer to CPU (readback)
  std::tie(output, memcpyElapsedTime) = vaip::tensor_proto_new_d3d12_gpu_to_cpu(bufferGPU, d3d12_device.Get(), cmdList.Get(),
                                                   byteSize, dxQueue);
...
template <typename TElement>
uploadReadback VAIExecutionProviderTest(TElement* inputs, int size) {
  ComPtr<ID3D12Device> d3d12_device;
...
TEST(VitisAIExecutionProviderTest, VAIPTest) {
  ...
  myfile << "tensor size,GPU->CPU [us], GPU->CPU memcpy [us],CPU->GPU [us] \n";
  myfile << "1920x1080 ~8M," << res_1920x1080.readback << "," << res_1920x1080.memcpyElapsedTime << "," << res_1920x1080.upload << ",\n ";
  myfile << "2560x1440 ~15M," << res_2560x1440.readback << "," << res_2560x1440.memcpyElapsedTime << "," << res_2560x1440.upload << ",\n ";
  myfile << "3840x1260 ~19M," << res_3840x1260.readback << "," << res_3840x1260.memcpyElapsedTime << "," << res_3840x1260.upload << ",\n ";
  myfile << "\n";

  myfile << "1D Performance: \n";
  myfile << "tensor size, int8 (GPU->CPU) [us], int8 (GPU->CPU memcpy) [us], int8 (CPU->GPU) [us],fp32 (GPU->CPU) [us], fp32 (GPU->CPU memcpy) [us], fp32 (CPU->GPU) [us] \n";
  myfile << "10K," << res_int8_10k.readback << "," << res_int8_10k.memcpyElapsedTime << "," << res_int8_10k.upload << "," << res_fp32_10k.readback << "," << res_fp32_10k.memcpyElapsedTime << "," << res_fp32_10k.upload
         << ",\n ";
  myfile << "100K," << res_int8_100k.readback << "," << res_int8_100k.memcpyElapsedTime << "," << res_int8_100k.upload << "," << res_fp32_100k.readback << "," << res_fp32_100k.memcpyElapsedTime << "," << res_fp32_100k.upload
         << ",\n ";
  myfile << "1M," << res_int8_1m.readback << "," << res_int8_1m.memcpyElapsedTime << "," << res_int8_1m.upload << "," << res_fp32_1m.readback << "," << res_fp32_1m.memcpyElapsedTime << "," << res_fp32_1m.upload
         << ",\n ";
  myfile << "10M," << res_int8_10m.readback << "," << res_int8_10m.memcpyElapsedTime << "," << res_int8_10m.upload << "," << res_fp32_10m.readback << "," << res_fp32_10m.memcpyElapsedTime << "," << res_fp32_10m.upload
         << ",\n ";
}
